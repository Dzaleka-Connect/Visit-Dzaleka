...
const store = vi.hoisted(() => ({ inventory: vi.fn(), recordActivity: vi.fn().mockResolvedValue(undefined), reserve: vi.fn() }));
...
function request(path: string, options: { authorized?: boolean; body?: unknown; query?: Record<string, string> } = {}) {
  ...
  }, {} as any) as Promise<{ statusCode: number; body: string; headers: Record<string, string> }>;
}
...
describe("dedicated GetYourGuide runtime", () => {
  ...
  it("uses current bookings and holds, persists telemetry, and prevents caching", async () => {
    ...
    expect(store.inventory).toHaveBeenCalledTimes(1);
  ...
  it("does not bundle application routes, sessions, email, or reporting into supplier cold starts", async () => {
    ...
    expect(Object.keys(result.metafile.inputs).filter(path => /server\/(app|routes|storage|email)\.ts|express-session|report-scheduler/.test(path))).toEqual([]);
  });
});
